...
This module provides functionality for detecting cars in images and videos
using the YOLOv8 model, with enhancements for license plate detection integration.

It includes features for:
- Detecting cars using YOLOv8
- Extracting regions of interest (ROIs) for license plates
...
# Car classes to detect (subset of vehicle classes)
CAR_CLASSES = {
    2: "Car"
}
...
# URL to download the default car detection model if missing
DEFAULT_CAR_MODEL_URL = "https://github.com/ultralytics/assets/releases/download/v8.2.0/yolov8n.pt"

# Global model cache
global_model = None

class CarDetector:
    """Car detection using YOLOv8 with enhanced features"""

    def __init__(self, model_path: str = "yolov8n.pt", conf_threshold: float = 0.5, use_gpu: bool = False):
        """
        Initialize the car detector with YOLOv8.
        ...
        if not os.path.exists(model_path):
            ...
            if not self.download_car_model(model_path):
    ...
    def download_car_model(self, model_path: str) -> bool:
        """
        Download the car detection model if it's missing.
        ...
        """
        try:
            logger.info(f"[INFO] Downloading car detection model to: {model_path}")
            response = requests.get(DEFAULT_CAR_MODEL_URL, stream=True, timeout=30)
            response.raise_for_status()
            with open(model_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        f.write(chunk)
            logger.info(f"[SUCCESS] Car detection model downloaded to: {model_path}")
            return True
        except Exception as e:
            logger.error(f"[ERROR] Failed to download car detection model: {e}")
            return False
    ...
    def estimate_plate_roi(self, bbox: Tuple[int, int, int, int], image_shape: Tuple[int, int]) -> Tuple[int, int, int, int]:
        """
        Estimate the region of interest (ROI) for a license plate within a car's bounding box.
        ...
        Returns:
            Estimated plate ROI (x1, y1, x2, y2)
        """
        x1, y1, x2, y2 = bbox
        car_width = x2 - x1
        car_height = y2 - y1

        # Estimate plate position: typically in the lower part of the car, centered horizontally
        plate_height_ratio = 0.1  # Plate height is ~10% of car height
        plate_width_ratio = 0.4   # Plate width is ~40% of car width
        plate_y_offset = 0.7      # Plate starts at ~70% of car height (lower part)

        plate_width = int(car_width * plate_width_ratio)
        plate_height = int(car_height * plate_height_ratio)
        plate_x1 = x1 + int((car_width - plate_width) / 2)  # Center horizontally
        plate_y1 = y1 + int(car_height * plate_y_offset)    # Lower part of car
    ...
    def detect(self, frame: np.ndarray, conf: Optional[float] = None, use_gpu: Optional[bool] = None) -> List[Dict[str, Any]]:
        """
        Detect cars in a frame with adaptive confidence thresholding.
        ...
        Returns:
            List of detections with bounding boxes, class information, and estimated plate ROIs
        """
        try:
                ...
                try:
                    ...
                    for box, conf, cls_id in zip(boxes, confidences, class_ids):
                        if cls_id in CAR_CLASSES:
                            x1, y1, x2, y2 = box
                            bbox = (x1, y1, x2, y2)
                            plate_roi = self.estimate_plate_roi(bbox, (original_h, original_w))
                            detections.append({
                                'bbox': bbox,
                                'class': CAR_CLASSES[cls_id],
                                'confidence': float(conf),
                                'plate_roi': plate_roi  # Add estimated plate ROI
                            })
            ...
            logger.info(f"[INFO] Detected {len(detections)} cars in frame")
            return detections
    ...
    def draw_detections(self, frame: np.ndarray, detections: List[Dict[str, Any]], car_counts: Dict[str, int], custom_colors: Optional[Dict[str, Tuple[int, int, int]]] = None, line_thickness: int = 2, font_scale: float = 0.6, overlay_stats: bool = True, show_plate_roi: bool = False) -> np.ndarray:
        """
        Draw detection boxes on the frame with enhanced visualization, optional statistics overlay, and optional plate ROIs.

        Args:
            frame: Input frame
            detections: List of detections
            car_counts: Dictionary with car counts by type
            custom_colors: Custom colors for different car types
            line_thickness: Thickness of bounding box lines
            font_scale: Scale of the font for labels
            overlay_stats: Whether to overlay detection statistics on the image
            show_plate_roi: Whether to draw estimated license plate ROIs
        ...
        for det in detections:
            ...
            cls = det['class']
            conf = det['confidence']
            plate_roi = det.get('plate_roi', None)
            ...
            # Draw estimated plate ROI if requested
            if show_plate_roi and plate_roi:
                px1, py1, px2, py2 = plate_roi
                cv2.rectangle(output_frame, (px1, py1), (px2, py2), (255, 0, 255), 1)  # Magenta for plate ROI
                cv2.putText(output_frame, "Plate ROI", (px1, py1 - 5), cv2.FONT_HERSHEY_SIMPLEX, font_scale * 0.5, (255, 0, 255), 1)

        # Overlay statistics if requested
        if overlay_stats:
            stats_text = [f"{cls_name}: {count}" for cls_name, count in car_counts.items() if count > 0 or cls_name == "Total"]
            y_offset = 30
...
try:
    global_model = CarDetector(model_path="yolov8n.pt", use_gpu=False)
...
# Reintroduce load_model for backward compatibility
def load_model(model_path: str = "yolov8n.pt", timeout: int = 30) -> Optional[Any]:
    """
    Load the YOLOv8 model (compatibility wrapper).

    Args:
        model_path: Path to the model file
        timeout: Maximum time to wait for model loading in seconds (not used, kept for compatibility)

    Returns:
        CarDetector instance or None if loading fails
    """
    global global_model
    if global_model is not None:
        return global_model
    try:
        global_model = CarDetector(model_path=model_path, use_gpu=False)
        return global_model
    except Exception as e:
        logger.error(f"[ERROR] Failed to load model: {str(e)}")
        return None

def get_detector() -> Optional[CarDetector]:
    """
    Get the global CarDetector instance.

    Returns:
        The global CarDetector instance, or None if not initialized.
    """
    return global_model
...
def generate_random_colors(num_colors: int = 5) -> Dict[str, Tuple[int, int, int]]:
    ...
    """
    colors = {}
    for cls_name in CAR_CLASSES.values():
        colors[cls_name] = (
...
def detect_cars(
    ...
    show_plate_roi: bool = False
) -> Tuple[List[Dict[str, Any]], Dict[str, int]]:
    """
    Detect cars in an image with enhanced visualization, statistics, and plate ROI estimation.
    ...
    Returns:
        Tuple containing list of car detections (with plate ROIs) and dictionary with car counts by type
    """
    ...
    car_counts = {cls_name: 0 for cls_name in CAR_CLASSES.values()}
    car_counts["Total"] = 0

    # Count cars by class
    for det in detections:
        class_name = det['class']
        if class_name in car_counts:
            car_counts[class_name] += 1
            car_counts["Total"] += 1

    if len(detections) == 0:
        logger.info(f"[INFO] No cars detected in image: {image_path}")
        return [], car_counts

    logger.info(f"[SUCCESS] Detected {len(detections)} car(s) in image: {image_path}")

    # Draw detections
    output_frame = detector.draw_detections(frame, detections, car_counts, colors, line_thickness, font_scale, overlay_stats, show_plate_roi)

    # Save cropped car images
    if save_path:
        cropped_dir = os.path.join(os.path.dirname(save_path), "cropped_cars")
        ...
        for i, det in enumerate(detections):
            ...
            try:
                ...
                logger.info(f"[INFO] Saved cropped car image to: {crop_path}")
            except Exception as e:
                logger.error(f"[ERROR] Failed to save cropped car image to {crop_path}: {str(e)}")
    ...
    logger.info(f"Car counts for {image_path}: " + ", ".join(stats_text))
    ...
    if show:
        ...
        try:
            cv2.imshow("Detected Cars", output_frame)
            cv2.waitKey(0)
...
def process_batch_file(args_tuple):
    ...
    try:
        ...
        car_count = counts["Total"]
        return {"filename": filename, "type": "image", "cars": car_count, "counts": counts, "detections": detections}
    except Exception as e:
        logger.error(f"[ERROR] Error processing {filename}: {e}")
        return {"filename": filename, "type": "image", "cars": 0, "counts": {}, "detections": []}
...
def batch_process(
    ...
    for filename in video_files:
        ...
        if not os.path.exists(filepath):
            ...
            results.append({"filename": filename, "type": "video", "cars": 0, "counts": {}, "detections": []})
            continue
        ...
        try:
            ...
            if result:
                ...
                cap.release()
                car_count = frame_count // 30  # Rough estimate: 1 car per 30 frames
                total_cars += car_count
                processed_videos += 1
                logger.info(f"[SUCCESS] Video processed. Estimated {car_count} cars.")
                results.append({"filename": filename, "type": "video", "cars": car_count, "counts": {"Total": car_count}, "detections": []})
            else:
    ...
    all_counts = {"Total": 0}
    for result in results:
        for car_type, count in result["counts"].items():
            if car_type != "Total":
                all_counts[car_type] = all_counts.get(car_type, 0) + count
        all_counts["Total"] += result["cars"]
    ...
    logger.info(f"[SUCCESS] Detected {total_cars} cars across all processed files.")
    logger.info(f"Car counts: {all_counts}")
